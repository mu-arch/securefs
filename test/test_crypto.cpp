...
TEST_CASE("Test AES-GCM with 96 bit IV")
{
    const byte key[] = {0x23, 0xff, 0xf4, 0xf5, 0x1e, 0xfb, 0x33, 0x28, 0x79, 0xcd, 0x41,
                        0x8a, 0x81, 0x6,  0xa4, 0xb5, 0x9,  0x81, 0xb5, 0xe5, 0xa2, 0x81,
                        0x91, 0xeb, 0x9a, 0x22, 0xaf, 0xe6, 0x98, 0xd5, 0xc0, 0x19};
    const byte iv[] = {0x1d, 0xb7, 0xbe, 0xe2, 0xeb, 0x22, 0x5d, 0xeb, 0x28, 0xb8, 0x8d, 0x64};
    const byte plaintext[]
        = {0x5d, 0x45, 0x4a, 0x3d, 0x22, 0x88, 0xb8, 0x32, 0xa,  0x8a, 0x1e, 0x8b,
           0x99, 0x3c, 0x2b, 0x1e, 0x2c, 0x30, 0x54, 0x59, 0x8b, 0x99, 0xc4, 0x5e,
           0xdd, 0x28, 0x16, 0x2,  0x1e, 0x78, 0x28, 0x6a, 0x95, 0xe0};
    const byte ciphertext[]
        = {0x75, 0x3d, 0x9a, 0x9,  0x6a, 0xec, 0x3b, 0x32, 0x14, 0x20, 0x35, 0x3d,
           0x1e, 0x8,  0xf8, 0x43, 0x1,  0x6d, 0xa3, 0x55, 0x41, 0xa,  0xc8, 0x50,
           0x62, 0xa6, 0x1b, 0x4d, 0x22, 0xde, 0x28, 0x5b, 0xf0, 0x93};
    const byte mac[] = {0x87,
                        0xf,
                        0x85,
                        0x28,
                        ...
                        0x37,
                        0x1c};
    securefs::AES_GCM enc(key, sizeof(key), securefs::CipherMode::ENCRYPT);
    byte computed_ciphertext[sizeof(ciphertext)];
    byte computed_mac[sizeof(mac)];

    enc.encrypt_and_authenticate(computed_ciphertext,
                                 computed_mac,
                                 sizeof(computed_mac),
                                 iv,
                                 sizeof(iv),
                                 ...
                                 plaintext,
                                 sizeof(plaintext));
    CHECK(memcmp(ciphertext, computed_ciphertext, sizeof(ciphertext)) == 0);
    CHECK(memcmp(mac, computed_mac, sizeof(mac)) == 0);
}
